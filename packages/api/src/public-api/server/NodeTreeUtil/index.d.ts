...
export interface NodeTreeUtil {
   ...
   */
  getNode(aNode: Node, aRelPath: string): Node;
   ...
   * @return the first occurrence of a sv:portlet with name <code>aPortletName</code>, or <code>null</code> if no such portlet can be found or if <code>aPageNode</code> is not a page node
   * @since Sitevision 2.6
   * @see #findPortletsByName(Node,String)
   */
  findPortletByName(aPageNode: Node, aPortletName: string): Node;
   ...
   * @param aPageNode the page node that has content, typically a sv:page or sv:article
   * @param aPortletName the name of the sv:portlet that should be found
   * @return all occurrences of portlets with name <code>aPortletName</code>. If no such portlets can be found or if <code>aPageNode</code> is not a page node, an empty <code>List</code> is returned
   * @since Sitevision 2.6
   * @see #findPortletByName(Node,String)
   */
  findPortletsByName(aPageNode: Node, aPortletName: string): unknown;
   ...
   * @return all occurrences of portlets with name <code>aPortletName</code> that also applies to the <code>aNodeFilter</code> filter. If no such portlets can be found or if <code>aPageNode</code> is not a page node, an empty <code>List</code> is returned
   ...
   * @return all occurrences of portlets of type <code>aPortletType</code>. If no such portlets can be found or if <code>aPageNode</code> is not a page node, an empty <code>List</code> is returned
   * @since Sitevision 3.6.2
   */
  findPortletsByType(aPageNode: Node, aPortletType: string): unknown;
   ...
   * @return all occurrences of portlets of type <code>aPortletType</code> that also applies to the <code>aNodeFilter</code> filter. If no such portlets can be found or if <code>aPageNode</code> is not a page node, an empty <code>List</code> is returned
   * @since Sitevision 3.6.2
   */
  findPortletsByType(
    aPageNode: Node,
   ...
   * @return a top-down list of the web nodes that are in the tree path from the site page node downto <code>aDescendantNode</code> (the site page node and the descendant node are both included in the list). An empty list will be returned if <code>aDescendantNode</code> is <code>null</code> or not a descendant of the site page.
   ...
   */
  getWebPathNodes(aDescendantNode: Node): unknown;
}
